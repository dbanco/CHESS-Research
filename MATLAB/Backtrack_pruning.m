...
results_dir = '/data/dbanco02/initial_fit/';
...
%% Backtrack Pruning
% completed step 0 11:200 >0.30
% completed step 1 11:35  >0.30
% completed step 1 36:200 >0.30
% completed step 2 all
% completed step 3 all
% completed step 0 all
...
for step = 1:1
    parfor img = 0:204
        ...
        err_struc = load([error_dir,... 
                ...
                num2str(step), '_img_',...
                num2str(img), '.mat']);
        if(err_struc.rel_fit_error <= 0.30 || img<11 || img>200)
            % Load polar_image
...
for img = 11:200
    ...
    try
        load(dir)
        disp(['Err ' num2str(rel_fit_error_old) ' to ' num2str(rel_fit_error_new)])
        disp(['||x||_0 ' num2str(sparse_old) ' to ' num2str(sparse_new)])
    end
end
