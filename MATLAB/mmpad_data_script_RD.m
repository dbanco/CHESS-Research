...
for i = 1:546
    ...
    load(fullfile('E:\PureTiRD_nr2_c_x39858\PureTiRD_nr2',fname))
    imagesc(mmpad_img)
    pause()
end
...
ring1 = img_array(:,2:262,5:45);
ring2 = img_array(:,2:262,220:260);
ring3 = img_array(:,2:262,268:308);
ring4 = img_array(:,2:262,355:395);
% Set ring
ring = ring3;
ringzzz = {ring1,ring2,ring3,ring4};

%% Save reduced images to file
imDir = 'E:\PurTiTD_nr1_c_x39858\';
for ring_num = 1:4
    ring_access = ringzzz{ring_num};
    fdir = sprintf('ring%i_zero',ring_num);
    mkdir(fullfile(imDir,fdir))
    for i = 1:546
        fname = sprintf('mmpad_img_%i.mat',i);
        polar_image = squeeze(ring_access(i,:,:))';
...
%% Display array
for j = 1:546
    imshow(squeeze((j,:,:)),'DisplayRange',[0,10e3],'ColorMap',jet)
    pause(0.1)
end
