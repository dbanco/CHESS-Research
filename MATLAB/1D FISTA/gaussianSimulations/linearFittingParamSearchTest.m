...
dataset =  fullfile(top_dir,dset_name);
output_dir  = fullfile(top_dir,output_subdir);
load(fullfile(output_dir,[dset_name,'_',num2str(20),'_','all2']))
...
for i = 1:M
    for time = 1:T
        ...
        mse_indep(i,time) = norm(fit-B(:,time)).^2;
        rse_indep(i,time) = norm(fit-B(:,time)).^2/norm(B(:,time)).^2;
...
err_lvl = linspace(0.02,0.2,NN);
% for time = 1:T
%     crit =  l1_norm(:,time) + mse_indep(:,time);
%     discrep_crit = abs(rse_indep(:,time) - err_lvl*ones(M,1));
%     select_ind(time) = find( (discrep_crit == min(discrep_crit)),1 );
...
for t = 1:T
    ...
    select_ind(t) = find( sq_origin_dist == min(sq_origin_dist + (err_t == 0) )  );
    
    % Relative Error threshold
    rel_err_t = rse_indep(:,t);
    while rel_err_t(select_ind(t)) > err_lvl(nn)
        if select_ind(t) > 1
            select_ind(t) = select_ind(t) - 1;
        else
            select_ind(t) = find(rel_err_t == min(rel_err_t));
            break
        end
    end
    
    % Compute AWMV for parameter selection
    x = X_indep(:,:,select_ind(t),t);
    az_signal = squeeze(sum(x,1));
    var_sum = squeeze(sum(az_signal(:)));
...
% ylabel('Criterion')
% subplot(2,1,2)
% loglog(l1_norm(:,time),rse_indep(:,time))
% hold on 
% loglog(l1_norm(select_ind(time),time),rse_indep(select_ind(time),time),'o')
% ylabel('RSE')
% xlabel('l_1 norm')
