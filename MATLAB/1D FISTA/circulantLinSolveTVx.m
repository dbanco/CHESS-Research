function [x_hat] = circulantLinSolveTVx( A0ft_stack,b,yk,vk,zk1,zk2,u1k,u2k,params,xEnd )
...
coef = diag(A0ft_stack*r_ft.')./((rho + (1 + xEnd)*rho2) + diag(A0ft_stack*A0ft_stack'));
x_ft = (r_ft - repmat(coef,[1,20]).*(A0ft_stack'.'))/(rho + (1 + xEnd)*rho2);
