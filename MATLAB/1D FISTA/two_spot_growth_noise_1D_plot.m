...
datasetArray = {'simulated_two_spot_1D_noise_independent_quadratic'};
noise_std = [0.08 0.1 0.12 0.14 0.16 0.18 0.2 0.22 0.24 0.26 0.28 0.3];

legend_str = cell(13,1);
k = 1;
for i = 1:numel(noise_std)
    g_vals = noise_std(i);
    legend_str{k} = num2str(g_vals);
    k = k + 1;
end
legend_str{end} = 'truth';

n = numel(datasetArray);
colors = jet(numel(legend_str)+1);
