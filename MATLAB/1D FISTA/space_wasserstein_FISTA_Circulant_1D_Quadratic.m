function [x_hat, err, t_k, L, obj, l_0] = space_wasserstein_FISTA_Circulant_1D_Quadratic(A0ft_stack,b,neighbors_vdf,D,x_init,params)
...
while keep_going && (nIter < maxIter)
   ...
   gradW = reshape(gradW,[t,1])./numel(neighbors_vdf);
    ...
    prev_f = f;
    f_data = 0.5*norm((b-fit)./sqrt(b_tilde))^2;
