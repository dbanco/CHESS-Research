...
load(fullfile(output_dir,[dset_name,'_',num2str(1),'_','CGTV']))
...
for nn = 1:11
    ...
    dataset =  fullfile(top_dir,dset_name);
    output_dir  = fullfile(top_dir,output_subdir);
    load(fullfile(output_dir,[dset_name,'_',num2str(nn),'_','CGTV']))
    % 100 trials
    for i = 1:M
        for time = 1:T
            x = X_coupled(:,:,i,time);
...
hold on
errorbar(noise_std,avg_awmv_indep,std_awmv_indep,...
    'LineWidth',2,...
    'color','blue',...
    'LineStyle','-')
errorbar(noise_std,avg_awmv_coup,std_awmv_coup,...
    'LineWidth',2,...
    'color','red',...
    'Linestyle','--')
