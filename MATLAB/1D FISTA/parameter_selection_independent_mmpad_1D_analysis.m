...
output_dir = '/cluster/shared/dbanco02/mmpad_subset_1D_indep_param_1/';
param_dir = '/cluster/shared/dbanco02/param_search_mmpad_1D/';
mkdir(param_dir)
mkdir(output_dir)
num_ims = 500;
...
for image_num = 1:num_ims
    ...
    for ii = 1:N
        ...
        P_new.set = ii;
        [x_hat,err,obj,~,~,~] = FISTA_Circulant_1D_Poisson(A0ft_stack,bn',x_init,P_new.params);
        save_output(output_dir,baseFileName,x_hat,err,im_data.polar_image,P_new,image_num)
    end
end
%save([param_dir,'param_search_mmpad_1D.mat'],'err_gcv','obj_gcv','P','lambda_vals')
