...
for ijk = [3]
    ...
    num_ims = 20;
    dset_fit = [dset_name,'_coupled_GD_TVx1\'];
    datadir = ['E:\CHESS_data\',dset_fit];
    dataset = ['E:\CHESS_data\simulated_two_spot_1D_',dset_name,'_',dataset_num,'\'];
    init_dir = ['E:\CHESS_data\','simulated_two_spot_1D_',dset_name,'_',dataset_num,'_simul_init_approx\'];
...
%     D2 = constructDistanceMatrix_1D(P,Threshold,maxNorm);
    ...
    for k = sM:eM
        fprintf('TV %i of %i\n',k,M)
        for j = 1:num_ims-1
            tv_dist = sum( sqrt( (x_hats{k,j}(:) - x_hats{k,j+1}(:)).^2 + tvBeta^2) );
            obj3(k,j) = tv_dist;
        end
    end
...
%     legend(legend_str,'location','best','FontSize',16)
    ...
    plot(gamma_vals(sM:eM),mean_err(sM:eM),'o-')
    ...
    plot(gamma_vals(sM:eM),mean_l1(sM:eM),'o-')
    ylabel('l1-norm')
    xlabel('Coupling parameter')
    ...
    hold on
    plot(gamma_vals(sM:eM),mean_l0(sM:eM),'o-')
    ylabel('l0-norm')
    xlabel('Coupling parameter')

    % Plot tv
    tv_time = mean(obj3,1);
    tv_total = mean(obj3,2);
    
    figure(44)
    semilogx(gamma_vals(sM),sum(tv_indep),'o')
    hold on
    semilogx(gamma_vals(sM:eM),tv_total(sM:eM),'o-')
    ylabel('TV')
    xlabel('Coupling parameter')
