function [X_hat, err, obj, l1_norm, tv_penalty] = convADMM_LASSO_CG_TVphi2_1D(A0ft_stack,B,X_init,params)
...
while keep_going && (nIter < maxIter)
    ...
    for t = 1:T
        Ykp1(:,:,t) = soft(alpha*Xkp1(:,:,t) + (1-alpha)*Yk(:,:,t) + Vk(:,:,t), lambda1(t)/rho);
    end
    if isNonnegative
        Ykp1(Ykp1<0) = 0;
    end
    ...
    Zkp1 = soft(DiffPhiX_1D(Xkp1) + Uk, lambda2/rho);
    ...
    if adaptRho
        sk = rho*sum((Ykp1(:)-Yk(:)).^2) + rho*sum((Zkp1(:)-Zk(:)).^2);
        rk = sum((Xkp1(:)-Ykp1(:)).^2) + sum((DiffPhiX_1D(Xkp1) - Zkp1).^2,'all');
        if sqrt(rk) > mu*sqrt(sk)
            rho = rho*tau;
        elseif sqrt(sk) > mu*sqrt(rk)
            rho = rho/tau;
        end
    end
