...
for ii = 1:NumLambdas
    ...
    P_local.set = ii;
    [X_hat, err, obj, l1_norm, tv_penalty] = convADMM_LASSO_CG_1D(A0ft_stack,B,X_init,params);
    try
        save_output(output_dir,baseFileName,X_hat,err,obj,im_data.polar_image,P_local);
    catch
        save_output(output_dir,baseFileName,X_hat,err,obj,im_data.polar_vector,P_local);
    end
end
...
function save_output(output_dir,baseFileName,X_hat,err,obj,polar_image,P)
    save(fullfile(output_dir,sprintf(baseFileName,P.set)),'X_hat','err','obj','polar_image','P');
end
