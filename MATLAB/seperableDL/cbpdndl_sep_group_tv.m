function [D, Y, Gh, Gv, optinf] = cbpdndl_sep_group_smooth(D0, D0h, D0v, S, lambda, opt)
...
while k <= opt.MaxMainIter && (rx > eprix|sx > eduax|rd > eprid|sd >eduad),
  ...
  optinf.itstat = [optinf.itstat;...
       [k Jfn Jdf Jl21 rx sx rd sd eprix eduax eprid eduad rho sigma tk]];
  if opt.Verbose,
    dvc = [k, Jfn, Jdf, Jl21, Jcn, rx, sx, rd, sd];
    if opt.AutoRho,
      dvc = [dvc rho];
    end
    if opt.AutoSigma,
