function [D, Y, Gh, Gv, optinf] = cbpdndl_sep_group_smooth(D0, D0h, D0v, S, lambda, kappa, opt)
...
%   NonnegativeDict  Force learned dictionary entries to be nonnegative
%
...
hstr = ['Itn   Fnc       DFid      l21      Dx     Cnstr     '...
        'r(X)      s(X)      r(D)      s(D) '];
...
while k <= opt.MaxMainIter && (rx > eprix|sx > eduax|rd > eprid|sd >eduad),
  ...
  Xf = solvemdbi_cg_smooth(Gf, rho, GSf + rho*fft2(Y - U), ...
                           cgt, opt.MaxCGIter, Xf(:), kappa);
%   Xf2 = solvedbi_sm(Gf, rho, GSf + rho*fft2(Y - U));
%   norm(Xf(:)-Xf2(:))
  ...
  % Solve Y subproblem
  Y = groupShrink(Xr + U, (lambda/rho)*opt.L1Weight);
  if opt.NonNegCoef,
    Y(Y < 0) = 0;
  end
  if opt.NoBndryCross,
  ...
  [N1,N2,K,T] = size(Y);
  YY = reshape(Y,[N1*N2*K,T]);
  Jl21 = opt.L1Weight*sum(sqrt(sum(sum(sum(Y.^2,1),2),3)),'all');
  ...
  % Compute smmothness term
  Jsmooth = 0.5*norm(Dxop(ifft2(Yf,'symmetric')))^2;
  ...
  clear Yf;
  Jfn = Jdf + lambda*Jl21 + kappa*Jsmooth;


  % Record and display iteration details
  tk = toc(tstart);
  optinf.itstat = [optinf.itstat;...
       [k Jfn Jdf Jl21 Jsmooth rx sx rd sd eprix eduax eprid eduad rho sigma tk]];
  if opt.Verbose,
    dvc = [k, Jfn, Jdf, Jl21, Jsmooth, Jcn, rx, sx, rd, sd];
    if opt.AutoRho,
      dvc = [dvc rho];
    end
    if opt.AutoSigma,
...
function u = groupShrink(v, lambda)
    [N1,N2,K,T] = size(v);
    vv = reshape(v,N1*N2*K,T);
    normVV = sqrt(sum(vv.^2,2));
    vv = (max(normVV-lambda,0)./normVV).*vv;
    u = reshape(vv,[N1,N2,K,T]);
return
