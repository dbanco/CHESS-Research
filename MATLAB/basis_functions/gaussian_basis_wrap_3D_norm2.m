...
dist_sq_rad = dist.^2;    % num_rad length vector

% Compute omega distance with wrapping
idx = 1:num_omega;
opposite = (idx(wrapN(mean_omega-floor(num_omega/2),num_omega)) +...
            idx(wrapN(mean_omega-ceil(num_omega/2),num_omega)))/2;
if opposite == mean_omega
    opposite = 0.5;
end
dist1 = abs(mean_omega - idx);
dist2 = num_omega/2 - abs(opposite - idx);
dist = min(dist1,dist2).*domega;
dist_sq_omega = dist.^2;    % num_omega length vector
...
all_dist_sq_theta = zeros(num_rad,num_theta,num_omega);
all_dist_sq_rad = zeros(num_rad,num_theta,num_omega);
all_dist_sq_omega = zeros(num_rad,num_theta,num_omega);

for i = 1:num_rad
    all_dist_sq_rad(i,:,:) = dist_sq_rad(i);
end
for j = 1:num_theta
    all_dist_sq_theta(:,j,:) = dist_sq_theta(j);
end 
for k = 1:num_omega
    all_dist_sq_omega(:,:,k) = dist_sq_omega(k);
end 
