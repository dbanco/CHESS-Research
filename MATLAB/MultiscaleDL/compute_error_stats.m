...
%           avg_Derror, std_Derror
    ...
    i = 1;
    for n = sig_ind
        error_stats.avg_error(i)      = mean(objectives{i}.error);
        error_stats.std_error(i)      = std(objectives{i}.error);
        error_stats.avg_rel_error(i)  = mean(objectives{i}.rel_error);
        error_stats.std_rel_error(i)  = std(objectives{i}.rel_error);
        error_stats.avg_true_error(i) = mean(objectives{i}.true_error);
        error_stats.std_true_error(i) = std(objectives{i}.true_error);
        error_stats.avg_Derror(i)     = mean(objectives{i}.D_error); 
        error_stats.std_Derror(i)     = std(objectives{i}.D_error); 
        error_stats.avg_of_penalty(i)     = mean(objectives{i}.of_penalty); 
        error_stats.std_of_penalty(i)     = std(objectives{i}.of_penalty); 
        error_stats.avg_hs_penalty(i)     = mean(objectives{i}.hs_penalty); 
        error_stats.std_hs_penalty(i)     = std(objectives{i}.hs_penalty); 
        error_stats.avg_ofhs_penalty(i)     = mean(objectives{i}.of_penalty+objectives{i}.hs_penalty); 
        error_stats.std_ofhs_penalty(i)     = std(objectives{i}.of_penalty+objectives{i}.hs_penalty); 
        i = i + 1;
    end
end
