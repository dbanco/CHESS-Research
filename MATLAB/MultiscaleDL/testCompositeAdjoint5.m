...
for N = 117:2:121   
    ...
    [Md,NormVals] = reSampleCustomArrayCenter(N,d,scales,center); 
    y = rand(1,N);
    ...
    ypad = padarray(Md,[0 M-1 0 0],0,'post');
    convty = ifft2(sum(bsxfun(@times, xh, fft2(ypad)), 4),'symmetric');
    Mtconvty = reSampleTransCustomArrayCenter(M,convty,scales,center,NormVals);
