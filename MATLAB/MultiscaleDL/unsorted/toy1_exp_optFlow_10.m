...
topDir = 'C:\Users\dpqb1\Documents\Outputs\toy1_exp_optFlow_10_Xtrue_Dindep_Vindep1';
...
close all
lambdas = [1e-2 10e-2];
lambda2s = [0.1 1 5 10 20 100 1e-3];
%% Dictionary learning
...
%     load('indepOutputs.mat')
    [u,v,~,~,~]  = computeHornSchunkDict(Xtrue,K,opt.Smoothness,opt.HSiters);
...
%     opt.Y0 = indepOutputs.X;
    opt.Y0 = Xtrue;
    [uIndep,vIndep,Fy,Fx,Ft]  = computeHornSchunkDict(Xindep,K,opt.Smoothness,opt.HSiters);
...
%     for j = 18 %1:numel(lambdas)
    ...
    for j = 7
        ...
        opt.rho2 = 50*lambda2;
        opt.UpdateVelocity = 1;
        [D,X,Dmin,Xmin,Uvel,Vvel,optinf,obj,relErr] = cbpdndl_cg_OF_multiScales_gpu(Dindep, y, lambda,lambda2, opt, scales,uIndep,vIndep);
