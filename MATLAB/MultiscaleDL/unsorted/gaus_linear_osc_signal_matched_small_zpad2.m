function [yn,y,K,J,N,M,T,Xtrue,Dtrue,scales] = gaus_linear_osc_signal_matched_small_zpad2(sigma)
...
scaling = '2-norm';
Dtrue = zeros(1,M,K);
Dtrue(1,:,1) = gaussian_basis_wrap_1D(M,25,sigMax,scaling);
Dtrue(1,6:widthMax+5,2) = linspace(0,10,widthMax)/norm(linspace(0,10,widthMax));
ADtrue = padarray(reSampleCustomArray(N,Dtrue,scales),[0,M-1,0,0],0,'post');
ADf = fft2(ADtrue);
...
minPos2 = -5;
amp = 15;
Pos  = M-1 + mod(round( amp*sin( 4*pi/50*((1:T)+12)/pFactor + pi/2 )      + amp + minPos1),N);
Pos2 =     M-1 + round( amp*sin( 4*pi/50*((1:T)+12)/pFactor + pi + pi/2 ) + amp + minPos2);
