...
% Directory
lambdaVals = [ 7e-2 8e-2 9e-2 5e-2 6e-2];
lambdaHSVals = [0.0005 0.001 0.002 0.003 0.01 0.1 1];
lambdaOFVals = [0    1e-3 2e-3 5e-3 1e-2,...
                ...
                17.5 35   40   50   75];
for iiii = 3
topDir = ['C:\Users\dpqb1\Documents\Outputs\toy4_exp_optFlow8_7_X0_D0_V0_zpad_HS',num2str(lambdaHSVals(iiii))];
...
% Data parameters
[y,~,K,J,N,M,T,~,~,scales] = gaus_linear_osc_signal_matched_small_zpad3(0);
y = reshape(y,[1,N,T]);
...
for i = 2%2:numel(sigmas)
    ...
    mkdir(figDir)
    % Data  
    [y,y_true,K,J,N,M,T,Xtrue,Dtrue] = gaus_linear_osc_signal_matched_small_zpad3(sigmas(i));
...
%     for j = 18 %1:numel(lambdas)
    for jj = 4:5
        for j = 1:25
            ...
            lambda = lambdaVals(jj);
            lambda2 = lambdaOFVals(j);     
            opt.UpdateVelocity = 1;
            
            last = load('C:\Users\dpqb1\Documents\Outputs\toy4_exp_optFlow8_7_X0_D0_V0_zpad_HS0.002_sig_2\output_j18_sig_1.00e-02_lam1_9.00e-02_lam2_2.00e+01.mat');
            D0 = last.outputs.D;
            opt.Y0 = last.outputs.X;

            [D,X,Dmin,Xmin,Uvel,Vvel,optinf,obj,relErr] = cbpdndl_cg_OF_multiScales_gpu_zpad(D0, y, lambda,lambda2, opt, scales,u,v);
            % Save outputs
            outputs = struct();
            outputs.y = y;
            outputs.D = D;
            ...
            % Generate figures
            generateFiguresToy1zpad(figDir,outputs,suffix,[4,4])
