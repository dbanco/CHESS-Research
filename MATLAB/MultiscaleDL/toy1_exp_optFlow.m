...
topDir = 'C:\Users\dpqb1\Documents\Outputs\toy1_exp_optFlow2_noJtermFixed2_alpha10';
...
opt.Smoothness = 10;%opt.Smoothness = 1e-8;
opt. HSiters = 30;
...
%     [D1,X1,Dindep,Xindep,~,~,~,~,~] = cbpdndl_cg_OF_multiScales_gpu(D0, y, lambda,lambda2, opt, scales,u,v,Jterm);
%     indepOutputs.y = y;
...
%     generateFiguresToy1(figDir,indepOutputs,suffixIndep)
    ...
    toc
%     tic
%     [u2,v2,Fy,Fx,Ft]  = computeHornSchunkDictLS(Xtrue,K,opt.Smoothness,opt.HSiters);
%     toc

    [uIndep,vIndep,Fy,Fx,Ft]  = computeHornSchunkDict(opt.Y0,K,opt.Smoothness,opt.HSiters);
    Jof = sum(vec(opticalFlowOp(Xtrue,u,v,K,0)).^2);
