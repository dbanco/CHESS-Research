...
% Directory
lambdaHSvals = [0.0001 0.001 0.002 0.003 0.01 0.1 1];
for iiii = 4
topDir = ['C:\Users\dpqb1\Documents\Outputs\toy2_exp_optFlow7_31_X0_Dfixed_V0_zpad_HS',num2str(lambdaHSvals(iiii))];
...
% Data parameters
[y,~,K,J,N,M,T,~,~,scales] = gaus_linear_osc_signal_matched_small_zpad(0);
y = reshape(y,[1,N,T]);
...
close all
lambdas = [1e-2 10e-2];
lambda2s = [0 1e-3 2e-3 5e-3 1e-2 2e-2 5e-2 0.1 0.2 0.5 1 2 4 6];
%% Dictionary learning
for i = 2%2:numel(sigmas)
    figDir = [topDir,'_sig_',num2str(i)];
    mkdir(figDir)
    % Data  
    [y,y_true,K,J,N,M,T,Xtrue,Dtrue] = gaus_linear_osc_signal_matched_small_zpad(sigmas(i));
...
%     for j = 18 %1:numel(lambdas)
    ...
    for j = 13
        ...
        lambda2 = lambda2s(j);     
        opt.UpdateVelocity = 1;
        [D,X,Dmin,Xmin,Uvel,Vvel,optinf,obj,relErr] = cbpdndl_cg_OF_multiScales_gpu_zpad(Dtrue, y, lambda,lambda2, opt, scales,u,v);
        % Save outputs
        outputs = struct();
        outputs.y = y;
        outputs.D = D;
