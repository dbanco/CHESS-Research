...
% Directory
topDir = 'C:\Users\dpqb1\Documents\Outputs\toy1_exp1';
dName = sprintf('gs_sq');
mkdir(topDir)
...
for i = 1:numel(sigmas)
    ...
    for j = 1:numel(lambdas)
        % Solve
        [D, X, optinf, obj, relErr] = cbpdndl_cg_multiScales(D0, y, lambdas(j), opt, scales);
        ...
        plotDataSeq(y,figDir,sprintf('y_seq_%i.gif',j))

        AD = reSampleCustomArray(N,D,scales);
        ADf = fft2(AD);
        Yhat = squeeze(ifft2(sum(bsxfun(@times,ADf,fft2(X)),3),'symmetric'));
        plotDataSeq(Yhat,figDir,sprintf('y_recon_%i.gif',j))
        close all
    end
end
