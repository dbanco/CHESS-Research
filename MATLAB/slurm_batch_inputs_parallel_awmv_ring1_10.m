...
% Output directories
dirA = fullfile('/cluster','shared','dbanco02','awmv_parallel_a_10');
dirB = fullfile('/cluster','shared','dbanco02','awmv_parallel_b_10');
mkdir(dirA)
mkdir(dirB)
...
jobDir1 = fullfile(datadir,['job_awmv_parallel_init_10']);
jobDir2 = fullfile(datadir,['job_awmv_parallel_ab_10']);
jobDir3 = fullfile(datadir,['job_awmv_parallel_ba_10']);
mkdir(jobDir1)
mkdir(jobDir2)
mkdir(jobDir3)
...
% Init script
slurm_write_matlab(numel(img_nums),jobDir1,'parallel_reg_awmv_FISTA_10','batch_script.sh')
