...
while keep_going && (nIter < maxIter)
    ...
    grad = ifft2(Atx_cpu(Df,Ax_cpu(Df,Zkf)),'symmetric') - c; % gradient of f at zk
    ...
    while ~stop_backtrack 
        ...
        fitX = ifft2(Ax_cpu(Df,Xkf),'symmetric');
        fitZ = ifft2(Ax_cpu(Df,Zkf),'symmetric');
    ...
    if params.plotProgress
        ...
        subplot(2,3,2)
        imshow(ifft2(Ax_cpu(Df,Xkf),'symmetric'),'DisplayRange',[lim1 lim2],'Colormap',jet);
        title('xk')
        ...
        subplot(2,3,4)
        fit_gk = ifft2(Ax_cpu(Df,fft2(gk)),'symmetric');
    ...
    switch stoppingCriterion
        case 'SUBGRADIENT'
            sk = L*(Xk-Xkm1) +...
                 Atx_cpu(Df,Ax_cpu(Df,fft2(Xk-Xkm1)));
