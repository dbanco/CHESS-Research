function [x_hat, err, obj, l_0] = space_wasserstein_FISTA_Circulant_Weighted(A0ft_stack,b,neighbors_vdf,D,x_init,params,w)
...
while keep_going && (nIter < maxIter)
    ...
    grad = AtR_ft_2D(A0ft_stack,forceMaskToZero(w.*Ax_ft_2D(A0ft_stack,zk),zMask)) - c;
   ...
   gradW = reshape(gradW,[t,r])./numel(neighbors_vdf);
    ...
    while ~stop_backtrack 
        ...
        temp1 = 0.5*norm(w(:).*b(:)-fit(:))^2 + 0.5*params.gamma*wObj_xk;
