function [x_hat, err, t_k, L, obj, l_0] = space_wasserstein_FISTA_Circulant_Poisson(A0ft_stack,b,neighbors_vdf,D,x_init,params)
...
f = 0.5*norm((b-Ax_ft_2D(A0ft_stack,x_init))./b_tilde)^2 +...
...
while keep_going && (nIter < maxIter)
    ...
    grad = AtR_ft_2D(A0ft_stack,forceMaskToZero(Ax_ft_2D(A0ft_stack,zk)./b_tilde,zMask)) - c;
   ...
   gradW = reshape(gradW,[t,r])./numel(neighbors_vdf);
    ...
    switch stoppingCriterion
        case STOPPING_SUBGRADIENT
            sk = L*(xk-xkm1) +...
                 AtR_ft_2D(A0ft_stack,Ax_ft_2D(A0ft_stack,forceMaskToZero(Ax_ft_2D(A0ft_stack,xk-xkm1)./b_tilde,zPad)));
