...
P.var_theta = linspace(1,10,P.num_var_t).^2;
P.var_rad   = linspace(1,  2,    P.num_var_r).^2;

A0ft_stack = unshifted_basis_matrix_ft_stack(P);
A0_stack = unshifted_basis_matrix_stack(P);
basis = squeeze(A0_stack(:,:,10,10));
[n,m] = size(basis); 
...
for k = 1:m
    subplot(5,5,k)
    imshow(bccb(n*(k-1)+1:n*k,1:n),'ColorMap',jet)
