function [D, Y, optinf, Jfn, relErr] = cbpdndl_cg_SqPhi_multiScales(D0, S, lambda, lambda2, opt, scales)
...
%     Xinit = squeeze(ifft2(Xfinit, 'symmetric'));
    ...
    [Xf, cgst] = solvemdbi_cg_SqPhi(AGf, rho, AGSf + rho*fft2(Y - U) ,...
        opt.CGTolX, opt.MaxCGIterX, Xinit(:),N2,KM,T,lambda2);
    cgIters2 = cgst.pit;
...
%   clear D;
  ...
  optinf.itstat = [optinf.itstat;...
       [k Jfn Jdf Jl1 Jtv rx sx rd sd eprix eduax eprid eduad rho sigma tk]];
  if opt.Verbose,
