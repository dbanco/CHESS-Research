...
if opt.StdResiduals,
    ...
    eprix2 = sqrt(Nz)*opt.AbsStopTol+max(nOFX,nZ)*opt.RelStopTol;
    eduax2 = sqrt(Nz)*opt.AbsStopTol+rho2*nV*opt.RelStopTol;
  else
    % See wohlberg-2015-adaptive
    rx2 = norm(vec(OFX - Z))/max(nOFX,nZ);
...
%           break
  ...
  optinf.itstat = [optinf.itstat;...
       [k Jfn Jdf Jl1 Jof rx sx rd sd eprix eduax eprid eduad rho rho2 sigma tk]];
  if opt.Verbose,
