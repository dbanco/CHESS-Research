function [D, Y, optinf, Jfn, relErr] = cbpdndl_cg_test_multiScales(D0, S, lambda, opt, scales)
...
Xinit = zeros(1,N2,totalScales,T);
% Main loop
k = 1;
...
%   Xf = solvedbi_sm(AGf, rho, AGSf + rho*fft2(Y - U));
  [Xf, cgst] = solvemdbi_cg_x(AGf, rho, AGSf + rho*fft2(Y - U),...
        opt.CGTolX, opt.MaxCGIterX, Xinit(:),N2,totalScales,T);
