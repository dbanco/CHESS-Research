function [D, Y, optinf] = cbpdndl_cg(D0, S, lambda, opt)
...
%   checkSolution(S,X,G,opt.numScales);
  ...
  AYS = decimateTrans(ifft2(sum(bsxfun(@times, conj(Yf), fft2(S)), 4),'symmetric'),opt.numScales);
...
% 	ifft2(sum(bsxfun(@times,Yf,fft2(decimate(D,opt.numScales))),3),'symmetric');
% 	decimateTrans(ifft2(sum(bsxfun(@times, conj(Yf), fft2(S)), 4),'symmetric'),opt.numScales);
    [D, cgst] = solvemdbi_cg_decim(Yf, sigma, AYS + sigma*(G - H), ...
