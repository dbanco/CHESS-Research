function [D, Y, optinf, Jfn, relErr] = cbpdndl_gcmp_cg_multiScales(D0, S, lambda, opt, scales)
...
Pgcmp.epsilon = 1;
Pgcmp.maxLayers = 1;
Pgcmp.NonnegativeDict = opt.NonnegativeDict;
Pgcmp.NonNegCoef = opt.NonNegCoef;
Pgcmp.DictFilterSizes = ones(2,totalScales);
j = 1;
for k = 1:K
    for i = 1:size(scales{k},2)
        scales_k = scales{k};
        Pgcmp.DictFilterSizes(2,j) = floor(opt.DictFilterSizes(2,k)*scales_k(1,i)/scales_k(2,i));
        j = j + 1;
    end
end
