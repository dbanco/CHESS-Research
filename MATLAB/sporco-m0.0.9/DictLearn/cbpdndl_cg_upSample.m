function [D, Y, optinf] = cbpdndl_cg_upSample(D0, S, lambda, opt)
...
K = size(D0,3);
if size(S,3) > 1,
  xsz = [size(S,1) size(S,2) size(D0,3)*opt.numScales size(S,3)];
  ...
  xsz = [size(S,1) size(S,2) size(D0,3)*opt.numScales 1];
end
Nx = prod(xsz);
...
AG = (upSample(G,opt.numScales));
AGf = fft2(AG);
AGSf = bsxfun(@times, conj(AGf), Sf);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%6
...
checkDict_upSample(X,G,opt.numScales,fdict);
  ...
  AYS = upSampleTrans(ifft2(sum(bsxfun(@times, conj(Yf), fft2(S)), 4),'symmetric'),opt.numScales);
...
%     test = upSampleTrans(ifft2(sum(bsxfun(@times, conj(Yf), fft2(S)), 4),'symmetric'),opt.numScales);
% 	test2 = ifft2(sum(bsxfun(@times,Yf,fft2(upSample(test,opt.numScales))),3),'symmetric');
    [D, cgst] = solvemdbi_cg_upSample(Yf, sigma, AYS + sigma*(G - H), ...
                      cgt, opt.MaxCGIter, D(:), opt);
    cgIters = cgst.pit;
    Df = fft2(D);
