function [D, Y, optinf] = cbpdndl_multiscale (D0,Phi, S, lambda, opt)
...
% is only 3d.
if size(S,3) > 1,
  xsz = [size(S,1) size(S,2) size(D0,3)*size(Phi,3) size(S,3)];
  ...
  xsz = [size(S,1) size(S,2) size(D0,3)*size(Phi,3) 1];
end
Nx = prod(xsz);
...
Phif = fft2(Phi,size(S,1),size(S,2));
PhifGf = scaleDict(Phif,Gf);
PhiGSf = bsxfun(@times, conj(PhifGf), Sf);
% Main loop
k = 1;
while k <= opt.MaxMainIter && (rx > eprix|sx > eduax|rd > eprid|sd >eduad),
  ...
  Xf = solvedbi_sm(PhifGf, rho, PhiGSf + rho*fft2(Y - U));
  ...
  Yf = fft2(Y);
  PhifYf = scaleCoefs(Phif,Yf);
  PhiYSf = sum(bsxfun(@times, conj(PhifYf), Sf), 4);
  ...
  if strcmp(opt.LinSolve, 'SM'),
    ...
    Df = solvemdbi_ism(PhifYf, sigma, PhiYSf + sigma*fft2(G - H));
  else
    [Df, cgst] = solvemdbi_cg(PhiYf, sigma, PhiYSf + sigma*fft2(G - H), ...
...
PhifGf = scaleDict(Phif,Gf);  
Jdf = sum(vec(abs(sum(bsxfun(@times,PhifGf,Yf),3)-Sf).^2))/(2*xsz(1)*xsz(2));
  clear Yf;
  Jfn = Jdf + lambda*Jl1;
...
function PhifXf = scaleCoefs(Phif,Gf)
[N1,N2,U] = size(Phif);
KU = size(Gf,3);
K=KU/U;
PhifXf = zeros(N1,N2,K,size(Gf,4));
for k = 1:K
    for u = 1:U   
        PhifXf(N1,:,k,:) = PhifXf(N1,:,k,:) +...
        bsxfun(@times,Phif(:,:,u),Gf(:,:,(u+(k-1)*U),:));
    end
end
return
