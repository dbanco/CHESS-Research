function [D, Ymin, optinf, Jfn, relErr] = cbpdndl_cg_TVphi_multiScales(D0, S, lambda, lambda2, opt, scales)
...
for i = 1:numel(scales)
    KM = KM + size(scales{i},2);
end

xsz = [size(S,1) size(S,2) KM T];
...
%     Xfinit = solvedbi_sm(AGf, rho, AGSf + rho*fft2(Y - U));
%     Xinit = squeeze(ifft2(Xfinit, 'symmetric'));
    
    [Xf, cgst] = solvemdbi_cg_TVphi(AGf, rho, AGSf + rho*fft2(Y - U) + rho2*fft2(PhiTranDiffTran_1D(Z - V,N2,KM,T-1)),...
        opt.CGTolX, opt.MaxCGIterX, Xinit(:),N2,KM,T,rho2);
    cgIters2 = cgst.pit;
  ...
  Yf = fft2(Y);
  DPYr = DiffPhiX_1D(Y,N2,KM,T);
  DPYr = reshape(DPYr,[1,1,KM,T-1]);

  % Solve Z subproblem
  Z = shrink(DPYr + V, (lambda2/rho2));
...
DPX = reshape(DiffPhiX_1D(X,N2,KM,T),[1,1,KM,T-1]);
nZ = norm(Z(:)); nDPX = norm(DPX(:)); nV = norm(V(:)); Nz = numel(Z);
if opt.StdResiduals,
    % See pp. 19-20 of boyd-2010-distributed
    rx2 = norm(vec(DPX - Z));
    sx2 = norm(vec(rho2*(Zprv - Z)));
    eprix2 = sqrt(Nz)*opt.AbsStopTol+max(nDPX,nZ)*opt.RelStopTol;
    eduax2 = sqrt(Nz)*opt.AbsStopTol+rho2*nV*opt.RelStopTol;
  else
    % See wohlberg-2015-adaptive
    rx2 = norm(vec(DPX - Z))/max(nDPX,nZ);
    sx2 = norm(vec(Zprv - Z))/nV;
    eprix2 = sqrt(Nz)*opt.AbsStopTol/max(nDPX,nZ)+opt.RelStopTol;
    eduax2 = sqrt(Nz)*opt.AbsStopTol/(rho2*nV)+opt.RelStopTol;
  end
  clear X DPX;
  ...
  % Compute TV of Y
  DPY = reshape(DiffPhiX_1D(Y,N2,KM,T),[1,1,KM,T-1]);
  Jtv = sum(abs(vec(DPY)));
  clear DPY;
...
%   clear D;
  ...
  clear Yf;
  Jfn = Jdf + lambda*Jl1 + lambda2*Jtv;
  if Jfn < minJfn
      Ymin = Y;
      Gmin = G;
      minJfn = Jfn;
...
%           break
  ...
  optinf.itstat = [optinf.itstat;...
       [k Jfn Jdf Jl1 Jtv rx sx rd sd eprix eduax eprid eduad rho rho2 sigma tk]];
  if opt.Verbose,
    dvc = [k, Jfn, Jdf, Jl1, Jtv, Jcn, cgIters1,cgIters2, rx, sx, rd, sd];
    if opt.AutoRho,
      dvc = [dvc rho rho2];
    end
    if opt.AutoSigma,
  ...
  if opt.AutoRho,
    if k ~= 1 && mod(k, opt.AutoRhoPeriod) == 0,
      ...
      rsf = 1;
      if rx2 > opt.RhoRsdlRatio*sx2, rsf = rhomlt; end
      if sx2 > opt.RhoRsdlRatio*rx2, rsf = 1/rhomlt; end
      rho2 = rsf*rho2;
      V = V/rsf;
    end
  end
