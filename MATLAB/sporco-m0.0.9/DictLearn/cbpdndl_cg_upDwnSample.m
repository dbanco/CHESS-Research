function [D, Y, optinf] = cbpdndl_cg_upDwnSample(D0, S, lambda, opt)
...
K = size(D0,3);
if size(S,3) > 1,
  xsz = [size(S,1) size(S,2) size(D0,3)*2*(opt.numScales+1) T];
...
%   checkSolution(S,X,G,opt.numScales);
  ...
  elseif strcmp(opt.LinSolve, 'CGD')
    ...
    [D, cgst] = solvemdbi_cg_upDwnSample(Yf, sigma, AYS + sigma*(G - H), ...
