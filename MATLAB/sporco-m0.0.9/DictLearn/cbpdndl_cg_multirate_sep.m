function [D,AG, Y, Gh, Gv, optinf, Jfn, relErr] = cbpdndl_cg_multirate_sep(D0,D0h,D0v, S, lambda, opt, c1,c2,Ufactors)
...
hstr = ['Itn   Fnc       DFid      l1        Cnstr     CGIters      '...
        'r(X)      s(X)      r(D)      s(D) '];
sfms = '%4d %9.2e %9.2e %9.2e %9.2e %4d %9.2e %9.2e %9.2e %9.2e';
nsep = 84;
if opt.AutoRho,
  hstr = [hstr '     rho  '];
  sfms = [sfms ' %9.2e'];
...
Pnrmh = @(x) bsxfun(@rdivide, x, sqrt(mean(sum(x.^2, 2), 1))); %%%% new
Pnrmv = @(x) bsxfun(@rdivide, x, sqrt(mean(sum(x.^2, 1), 2))); %%%% new
...
Pzpv = @(x) x; %%%% new
Pzph = @(x) x; %%%% new
PzpvT = @(x) bcrop(x, [dsz(1) size(x,2)]); %%%% new
PzphT = @(x) bcrop(x, [size(x,1) dsz(2)]); %%%% new
...
elseif opt.NonnegativeDict,
  ...
  Pcnv = @(x) removeNan(Pnrmv(Pzpv(PzpvT(Pnonneg(x)))));
  Pcnh = @(x) removeNan(Pnrmh(Pzph(PzphT(Pnonneg(x)))));
else
...
%   checkSolution_upDwnSample(S,Y,G,opt.numScales,fsol);
...
%   checkDict_upDwnSample(X,G,opt.numScales,fdict);
 ...
 %----Dv----%
  Yauxf = bsxfun(@times, AGfh, Yfh);
  AYS = reSampleNuTrans2d(M1,ifft2(sum(sum(bsxfun(@times, conj(Yauxf), fft2(S)),2),4),'symmetric'),c1,c2,Ufactors,NormVals);
  [Dv, cgst] = solvemdbi_cg_multirate_v(Yauxf, sigma, AYS + sigmav*(Gv - Hv), ...
                      cgt, opt.MaxCGIter, Dv(:), opt,c1,c2,Ufactors,sqrt(NormVals));
                                      %^this Dvf?^
  cgIters = cgst.pit;
  ...
  %----Dh----%
  Yauxf = bsxfun(@times, AGfv, Yfv); % iffting and ffting again might be need for some Reason???
  AYS = reSampleNuTrans2(M2,ifft2(sum(sum(bsxfun(@times, conj(Yauxf), fft2(S)),1),4),'symmetric'),c1,c2,Ufactors,NormVals);
  [Dh, cgst] = solvemdbi_cg_multirate_h(Yauxf, sigmah, AYS + sigmah*(Gh - Hh), ...
                      cgt, opt.MaxCGIter, Dh(:), opt,c1,c2,Ufactors,sqrt(NormVals));
                                      %^this Dhf?^
  cgIters = cgst.pit;
  Dfh = fft(Dh,[],2);
...
%     AD = reSampleNu(N2,D,c1,c2,Ufactors);
...
%     pause(0.01)
  ...
  if opt.Verbose,
    dvc = [k, Jfn, Jdf, Jl1, Jcn, cgIters, rx, sx, rd, sd];
    if opt.AutoRho,
      dvc = [dvc rho];
    end
    if opt.AutoSigma,
