function [x, cgst] = solvemdbi_cg_upDwnSample(ah, rho, b, tol, mit, isn, opt)
...
U = 2*opt.numScales+1;
asz = [size(ah,1) size(ah,2)/2^opt.numScales size(ah,3)/U];
Aop = @(in) ifft2(sum(bsxfun(@times,ah,fft2(upDwnSample(in,opt.numScales))),3),'symmetric');
Ahop = @(in,norms) upDwnSampleTrans(ifft2(sum(bsxfun(@times, conj(ah), fft2(in)), 4),'symmetric'),opt.numScales);
