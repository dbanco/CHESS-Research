function [x, cgst] = solvemdbi_cg_OF(a, rho, b, tol, mit, isn,N,K,J,T,lambda2,U,V)
...
AhAvop = @(u) vec(ifft2(Ahop(Aop(fft2(reshape(u, xsz)))),'symmetric'));

AhAvop2 = @(u) vec(opticalFlowOp(reshape(u,xsz),U,V,K,1));
