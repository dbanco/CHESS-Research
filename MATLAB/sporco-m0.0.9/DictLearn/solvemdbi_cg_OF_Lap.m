function [x, cgst] = solvemdbi_cg_OF_Lap(a, rho, b, tol, mit, isn,N,K,J,T,lambda2)
...
AhAvop2 = @(u) vec(fft( opticalFlowOpLap(ifft2(reshape(u,xsz),'symmetric'),K,1) ));

wrn = warning('query','MATLAB:ignoreImagPart');
warning('off', 'MATLAB:ignoreImagPart');
[xv,flg,rlr,pit] = pcg(@(u) AhAvop(u) + rho*u + lambda2*AhAvop2(u), b(:), tol, mit, [], [], isn);
