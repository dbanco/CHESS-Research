function [x, cgst] = solvemdbi_cg_upSample(ah, rho, b, tol, mit, isn, opt)
...
asz = [size(ah,1) size(ah,2)/2^(opt.numScales-1) size(ah,3)/opt.numScales];
Aop = @(in) ifft2(sum(bsxfun(@times,ah,fft2(upSample(in,opt.numScales))),3),'symmetric');
Ahop = @(in,norms) upSampleTrans(ifft2(sum(bsxfun(@times, conj(ah), fft2(in)), 4),'symmetric'),opt.numScales);
AhAvop = @(in) vec(Ahop(Aop(reshape(in, asz))));
