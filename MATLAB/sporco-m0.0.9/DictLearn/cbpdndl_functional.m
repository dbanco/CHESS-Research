function [D, Y, optinf] = cbpdndl_functional (D0, S, lambda, opt)
...
% is only 3d.
if size(S,3) > 1,
  xsz = [size(S,1) size(S,2) opt.DictSize size(S,3)];  %%%%%%%%%%%%%%%%%%%%1
  ...
  xsz = [size(S,1) size(S,2) opt.numEntries 1];
end
Nx = prod(xsz);
Nd = size(D0,3)*size(D0,2);   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%2
cgt = opt.CGTol;
...
while k <= opt.MaxMainIter && (rx > eprix|sx > eduax|rd > eprid|sd >eduad),
  ...
  Xf = solvedbi_sm(AGf, rho, AGSf + rho*fft2(Y - U));%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%7
  ...
  Yf = fft2(Y, size(S,1), size(S,2));%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 8 Here we prepare things for D subproblem
  A = @(in,type) AfuncDecim(in,type,opt.numScales,Yf,N2,T,K,sigma); 
  tol = 1e-6;
  maxIters = 100;
  c = [S(:);sqrt(sigma)*(G(:) - H(:))]; %  
  
  [D1,~,cgitr] = lsqrSOL(N2*T+N2*K,N2*K,A,c,G(:),0,tol,tol,1e5,maxIters,0);
  D = reshape(D1,[N1,N2,K,1]);
  ...
  optinf.itstat = [optinf.itstat;...
       [k Jfn Jdf Jl1 Jcn cgitr rx sx rd sd eprix eduax eprid eduad rho sigma tk]];
  if opt.Verbose,
    dvc = [k, Jfn, Jdf, Jl1, Jcn, cgitr, rx, sx, rd, sd];
    if opt.AutoRho,
      dvc = [dvc rho];
    end
    if opt.AutoSigma,
