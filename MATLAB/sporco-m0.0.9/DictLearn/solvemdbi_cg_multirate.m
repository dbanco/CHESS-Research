function [x, cgst] = solvemdbi_cg_multirateCustom(ah, rho, b, tol, mit, isn,opt,c1,c2,Ufactors,NormVals)
...
AhAvop = @(in) vec(wrapAhA(reshape(in, asz),N2,c1,c2,Ufactors,ah,M,NormVals));
...
function out = wrapAhA(in,N2,c1,c2,Ufactors,ah,M,NormVals)
A1 = reSampleNu(N2,in,c1,c2,Ufactors,NormVals);
Ain = sum(bsxfun(@times,ah,fft2( A1 )),3);
out = reSampleNuTrans2(M,ifft2(sum(bsxfun(@times, conj(ah), Ain), 4),'symmetric'),c1,c2,Ufactors,NormVals);
end
