function [D, Y, optinf, Jfn, relErr] = cbpdndl_cg_multirate(D0, S, lambda, opt, c1,c2,Ufactors)
...
[AG,NormVals] = reSampleNu(N2,G,c1,c2,Ufactors);
AGf = fft2(AG);
...
%   checkDict_upDwnSample(X,G,opt.numScales,fdict);
  ...
  elseif strcmp(opt.LinSolve, 'CGD')
    % REPLACE
%     test = upDwnSampleTrans(ifft2(sum(bsxfun(@times, conj(Yf), fft2(S)), 4),'symmetric'),opt.numScales);
% 	test2 = ifft2(sum(bsxfun(@times,Yf,fft2(upDwnSample(test,opt.numScales))),3),'symmetric');
    % REPLACE
    [D, cgst] = solvemdbi_cg_multirate(Yf, sigma, AYS + sigma*(G - H), ...
                      cgt, opt.MaxCGIter, D(:), opt,c1,c2,Ufactors,NormVals);
    cgIters = cgst.pit;
    Df = fft2(D);
  ...
  G = Pcn(Dr + H);
  % REPLACE
  %AG = upDwnSample(G,opt.numScales);
  [AG,NormVals] = reSampleNu(N2,G,c1,c2,Ufactors);
  AGf = fft2(AG);
  AGSf = bsxfun(@times, conj(AGf), Sf);
