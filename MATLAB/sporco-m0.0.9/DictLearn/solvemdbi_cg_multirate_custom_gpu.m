function [x, cgst] = solvemdbi_cg_multirate_custom_gpu(ah, rho, b, tol, mit, isn,M,scales,NormVals)
...
Ain = sum(pagefun(@times,ah,fft2( A1 )),3);
out = reSampleTransCustomArray(M,ifft2(sum(pagefun(@times, a, Ain), 4),'symmetric'),scales,NormVals);
end
