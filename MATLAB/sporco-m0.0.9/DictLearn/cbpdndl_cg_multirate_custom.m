function [D, Y, optinf, Jfn, relErr] = cbpdndl_cg_multirate_custom(D0, S, lambda, opt, scales)
...
M = N0;
K = size(D0,3);
numScales = size(scales,2);
if size(S,3) > 1,
  xsz = [size(S,1) size(S,2) size(D0,3)*numScales T];
...
[AG,NormVals] = reSampleCustom(N2,G,scales);
AGf = fft2(AG);
...
%   checkDict_upDwnSample(X,G,opt.numScales,fdict);
  ...
  AYS = reSampleTransCustom(M,ifft2(sum(bsxfun(@times, conj(Yf), fft2(S)), 4),'symmetric'),scales,NormVals);
