function [x, cgst] = solvemdbi_cg_multirate_v(ah, rho, b, tol, mit, isn, opt,c1,c2,Ufactors,NormVals)
...
AhAvop = @(in) vec(wrapAhA(reshape(in, asz),N1,c1,c2,U,ah,M1,NormVals));
...
Ain = sum(bsxfun(@times,ah,fft(A1)),3);
out = reSampleNuTrans2d(M,ifft2(sum(sum(bsxfun(@times, conj(ah), Ain),2),4),'symmetric'),c1,c2,U,NormVals);
end
