function [D, Y, Dmin,Ymin,Uvel,Vvel,optinf, Jfn, relErr] = cbpdndl_cg_OF_multiScales_gpu3(D0, S, lambda, lambda2, opt, scales,Uvel,Vvel)
...
Jlg2 = sigma*sum((H(:)).^2);
if lambda2 > 0
    [~,~,Fx,Fy,Ft] = computeHornSchunkDictPaperLS3(Y,K,Uvel,Vvel,opt.Smoothness,opt.HSiters);
...
%     Jdf3 = sum(vec(abs(recon-Sf).^2))
    ...
    [D, cgst] = solvemdbi_cg_multirate_custom_gpu(Yf, sigma, AYS + sigma*(G - H),...
                      cgt, opt.MaxCGIter, D(:),M,scales,NormVals);
    cgIters1 = cgst.pit;
    Df = fft2(D);
    ...
    if (opt.UpdateVelocity && (lambda2 > 0)) || nargin < 6
        [Uvel,Vvel,Fx,Fy,Ft] = computeHornSchunkDictPaperLS3(Y,K,Uvel,Vvel,opt.Smoothness,opt.HSiters);
    end
    ...
    if opt.plotDict
        AG = reSampleCustomArray(N2,G,scales);
...
function v = shrinkHard(v, lambda)
