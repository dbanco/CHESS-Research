...
for image_num = 1:num_ims
    ...
    for i = 1:P.num_var_t
        for j = 1:P.num_var_r
            x_init(:,:,i,j) = bn/(P.num_var_t*P.num_var_r);
        end
    end 
    ...
    parfor ii = 1:N
        ...
        P_new.set = ii;
        [x_hat,err,obj,~,~,~] = FISTA_Circulant_Poisson(A0ft_stack,bn,x_init,P_new.params);
        save_output(output_dir,baseFileName,x_hat,err,polar_image,P_new,image_num)
    end
end
save([param_dir,'param_search_mmpad_subset.mat'],'err_gcv','obj_gcv','P','lambda_vals')
