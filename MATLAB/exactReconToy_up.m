...
[y,yd,X_true,N,T,U] = upSample_translate_problem;
y = reshape(y,[1,N,1,T]);
...
%     D0(:,:,i) = D0(:,:,i)/norm(D0(:,:,i));
% end
D0(1,:,1) = yd(1,1:N0,1)+ 100*rand(1,N0,K)/100;

% Define Dictionary function
opt.DictSize = opt.numScales*K;
XAd = @(in,xf) ifft2(sum(bsxfun(@times,xf,fft2(upSample(in,opt.numScales))),3),'symmetric');
AtXty = @(in,xf) upSampleTrans(ifft2(sum(bsxfun(@times, conj(xf), fft2(in)), 4),'symmetric'),opt.numScales);

% Check init solution (good)
opt.Y0 = X_true;
Pnrm = @(x) bsxfun(@rdivide, x, sqrt(sum(sum(x.^2, 1), 2)));
% 
AD0 = upSample(D0,opt.numScales);
AD0f = fft2(AD0);
...
checkDict_upSample(X_true,D0,opt.numScales);

%% Dictionary learning
opt.LinSolve = 'CGD';
[D, X, optinf] = cbpdndl_cg_upSample(D0,y,lambda,opt);
Df = fft2(D);
Xf = fft2(X);
% For regular dictionary
% Yhat = squeeze(ifft2(sum(bsxfun(@times,Df,Xf),3),'symmetric'));
% For decimated
AD = (upSample(D,opt.numScales));
ADf = fft2(AD);
Yhat = squeeze(ifft2(sum(bsxfun(@times,ADf,Xf),3),'symmetric'));

topDir = 'C:\Users\dpqb1\Desktop\multiDict_Up\';
dName = 'exactReconToy_Up';
mkdir(topDir)
f1 = plotDictUsage(AD,K,opt.numScales,X,1);
