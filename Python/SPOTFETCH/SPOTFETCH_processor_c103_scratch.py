...
spotsPath = os.path.join(topPath_mnt,'spots','spots.npz') 
# spotsPath = os.path.join(topPath_nfs,'spots','spots.npz') 
spotData = np.load(spotsPath)
K = len(spotData['etas'])

# Template for getting groups of spots
grain = 5
eta = np.pi/2
deta = 0.1
spotInds1 = sf.findSpots(spotData, grain, eta, deta)

spotInds = np.arange(0,200) 
