...
exsituPath = os.path.join(dataPath,f'c103-3-ungripped-2_{num1:0>4}_EIG16M_CdTe_{num2:0>6}.h5')
dataFile = os.path.join(dataPath,"c103-3-ff-1_*_EIG16M_CdTe_{num2:0>6}.h5")
...
# frame = 2
# sf.plotSpotWedges(spotData,exsituPath,frame,params)
...
eta2 = 0
# grains1 = [5]
spotInds1 = sf.findSpots(spotData,eta=np.pi/2,deta=1,tth=tth_ring,dtth=tth_ring)
# spotInds2 = sf.findSpots(spotData,eta=np.pi,deta=10,tth=tth_ring,dtth=dtth_ring)

# # %% 6. Begin Processing
initTracksPath = os.path.join(topPath,'outputs')
sf.initExsituTracks(initTracksPath,exsituPath, spotData, spotInds1, params, 0)

num1 = 30
num2 = 300
sf.spotTracker(dataFile,topPath,spotData,spotInds1,params,num1,num2)