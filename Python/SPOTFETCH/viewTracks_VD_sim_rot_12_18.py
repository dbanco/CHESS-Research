...
spotInds = np.arange(10000)
nanFileName = r"E:\VD_sim_processing\nanInds_50000.pkl"
with open(nanFileName, "rb") as file:  # Open file in binary write mode
    nanInds = pickle.load(file)

# resultsData = sf.trackingResultsSim(spotInds,spotsFiles,scanRange,ttPath,params)

# # # Serialize and save the data
filename = os.path.join(topPath,"results_12_18_rot_10000.pkl")
...
# %% Sort by truthDist
truthDist = resultsData['truthDist']
truthDist[nanInds[:37],:] = np.nan
sumTruthDist = np.nansum(resultsData['truthDist'],1)
sumTruthDist2 = np.sum(resultsData['truthDist'],1)
spotIndsSort = np.argsort(sumTruthDist)[::-1]
sortTruthDist = sumTruthDist[spotIndsSort]

# # %% Make track image files
# output_path = os.path.join(topPath,'imageFigs_12_18_rot')
# dome = 4
# num_cols = 5
# sf.makeTrackImages(dome,num_cols,output_path,spotIndsSort,spotData,scanRange,dataFile,ttPath,spotsFiles,params)

# # %% Make image/Data table slides
# ppt_file = os.path.join(output_path, "track_slides_12_18_rot.pptx")
# sf.makeTrackSlides(ppt_file,output_path,spotIndsSort,resultsData,spotData)

# %% Compute fraction crect detections
total = len(truthDist.ravel())
correctDetects = np.sum(truthDist< 2e-3)
nanDetects = np.sum(np.isnan(sumTruthDist2))
